...
class AttentionCnnDwac(object):
    ...
    def evaluate(self, x, y, ref_loader):
        ...
        with torch.no_grad():
            ...
            total_correct += torch.eq(output['probs'].argmax(dim=1), y)
            return output
...
class AttentionCnnDwacModule(nn.Module):
    ...
    def forward(self, x, y):
        ...
        output_dict = self.classify_against_ref(z, norm, self.prototypes, self.prototypes_classes)
        class_dists = output_dict['class_dists']  # [batch_size, n_classes]
